...
class GeneVocab:
    def __init__(
        self,
        gene_list_or_vocab: Union[List[str], Vocab, "GeneVocab"],
        ...
        if isinstance(gene_list_or_vocab, list):
            self._vocab = self._build_vocab_from_iterator(
                gene_list_or_vocab,
                specials=specials,
                special_first=special_first
            )
        elif isinstance(gene_list_or_vocab, Vocab):
            self._vocab = gene_list_or_vocab
        elif isinstance(gene_list_or_vocab, self.__class__):
            self._vocab = gene_list_or_vocab._vocab
            if default_token is None:
                self.default_token = gene_list_or_vocab.default_token
        else:
            raise ValueError(
                "gene_list_or_vocab must be a list of gene names or a Vocab object. "
                f"Got '{type(gene_list_or_vocab)}'."
            )

        self.default_token = default_token
        self.default_index = self._vocab[default_token]

    def __call__(self, keys: List[str]) -> List[int]:
        return [self._vocab[k] for k in keys]
    ...
    def get_default_index(self) -> int:
        return self._vocab[self.default_token]
    ...
    def _build_vocab_from_iterator(
        ...
        word_vocab = Vocab()
        for ix, k in enumerate(ordered_dict.keys()):
            word_vocab[k] = ix

        return word_vocab

    @classmethod
    def from_dict(cls, token2idx: Dict[str, int], default_token: Optional[str] = "<pad>") -> Self:
        _vocab = Vocab()
        ...
        if default_token is not None and default_token in _vocab:
            new.default_token = default_token
    ...
    def save_json(self, file_path: Union[Path, str]) -> None:
        ...
        with file_path.open("w") as f:
            d = {k: self._vocab[k] for k in self._vocab.keys()}
